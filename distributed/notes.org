...
*** Algoritmo de consenso detección temprana
    ...
    *Acuerdo:* Sea \(r\) la primera ronda en que para algún \(p_i\) se cumple
    ...
    \(vec\_ant_r =​= vec\_act_r\). Por demostrar: \(prop_i == prop_j\).
    Por contradicción, supongamos que \(prop_i \neq prop_j\) en la ronda
    \(r,\ \Rightarrow,\ \exists\ p_k\ |\ p_k\ send_r(prop_k)\ \rightarrow\ p_i\ \wedge\
    p_k\ ¬send_r(prop_k) \rightarrow p_j\). Como \(p_i\) y \(p_j\) detectan
    que \(vec\_ant_r == vec\_ant_r\ \Rightarrow\ p_k\ send_{r-1}(prop_k)\
    \rightarrow\ \{p_i, p_j\} \Rightarrow p_j\) no recibió el mismo número de
    mensajes en dos rondas consecutivas.
    ...
    #+begin_proof
    Dada una \(t < f\) (número de fallas reales), en el peor de los casos, hay una
    falla en cada una de las t rondas. Entonces, en la ronda \(t + 1\) no hay
    fallas y se detecta el acuerdo. En la ronda \(t + 2\) todos deciden.

    Si t = f, se cumple que los procesos correctos terminan en la ronda
    f + 1. Esto si en cada ronda falla un proceso.
    Por otro lado, existen ejecuciones en las que \(t = f\), pero aún así, el
    algoritmo decide con mucho menos rondas que \(f + 1\). Por ejemplo, todos
    los procesos fallidos mueren al inicio y en tres rondas deciden.
    #+end_proof
    #+end_affirmation
